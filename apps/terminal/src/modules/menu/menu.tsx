...
const MenuModule = () => {
    ...
    const getAllAvaiables = () => {
        const avaiables = Dishes.data.filter((dish)=> dish.availability === true)
        return avaiables.length
    }

    const getAllNotAvaiables = () => {
        const notAvaiables = Dishes.data.filter((dish)=> dish.availability === false)
        ...
        <div className="w-full h-full flex flex-col items-center justify-center">
            <div className="flex flex-col w-full items-center justify-center">
                <div className="w-full bg-gray-100 py-12 px-4 sm:px-6 lg:px-8 flex flex-col gap-4">
                    <p className='font-bold text-[20px] text-gray-900 self-start '>Manage My Menu</p>
                    <div className="w-full flex flex-row items-center justify-between ">
                        ...
                        <button onClick={() => OpenOrCloseModal(true)} className="bg-gray-900 shadow-sm font-bold  gap-2 flex flex-row items-center  text-[14px] rounded-md py-[4px] px-[25px]">
                                                    ...
                                                    (
                                                        Dishes.data.map((platillo) => (
                                                            <tr className="text-gray-700" key={platillo.id}>
                                                                <td className="px-6 py-4 whitespace-nowrap">{platillo.name}</td>
                                                                <td className="px-6 py-4 whitespace-nowrap">${platillo.price}</td>
                                                                <td className="px-6 py-4 truncate max-w-[250px]">{platillo.desc}</td>
                                                                <td className="px-6 py-4 whitespace-nowrap">
                                                                    {platillo.availability ? (
                                                                        <span className="px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs">Available</span>
                                                                    ) : (
                                                                        <span className="px-3 py-1 rounded-full bg-red-100 text-red-700 text-xs">Not Available</span>
                                                                    )}
                                                                </td>
                                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-4">
                                                                    <button onClick={() => selectDishEdit(platillo.id)} className="p-2 border rounded-md hover:bg-gray-200">
                                                                        <CiEdit size={20} className="text-gray-600" />
                                                                    </button>
                                                                    <button onClick={() => deleteDish(platillo.id)} className="p-2 border rounded-md hover:bg-gray-200">
            ...
            <CreateOrEditDishModal addNewDish={addNewDish} editDish={editDish} dishData={selectedDish} isOpen={modalIsOpen} onClose={() => OpenOrCloseModal(true)} />
