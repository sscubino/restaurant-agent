...
const CreateOrEditDishModal = ({
  ...
  const handleCreateDish = async () => {
    try {
      if (createDish.name && createDish.price && createDish.description) {
        const data = await api.dishes.create({
          name: createDish.name,
          price: Number(createDish.price),
  ...
  const validateErrors = () => {
    ...
    if (!createDish.description)
      errorsForm.description = "Please enter a valid description";
  ...
  return (
    ...
    >
      <form
        onSubmit={handleValidFunctions}
        className="h-full w-[250px] flex flex-col items-center"
      >
        <button onClick={onClose} className="cursor-pointer self-start">
          <IoCloseOutline size={20} className="text-gray-800" />
        </button>
        <p className="text-[20px]  text-center font-bold text-gray-800">
          Create Dish
        </p>
        <Input
          label="Name"
        ...
        {dishData && (
          <div className="w-full flex flex-col mb-4">
            ...
            <select
              value={createDish?.isAvailable?.toString()}
              onChange={(e) =>
                handleChangeInput("isAvailable", e.target.value === "true")
              }
        ...
        <div className="w-full flex flex-col items-start">
          ...
          <textarea
            ...
            onBlur={() => setErrors((prev) => ({ ...prev, description: "" }))}
          ></textarea>
          {errors?.description && (
        ...
        <button
          style={{ background: !hasChanges ? "gray" : "black" }}
          disabled={!hasChanges}
          type="submit"
          role="button"
          className="w-full mt-5 rounded-md py-[8px]"
        >
          <p className="text-white font-bold">{dishData ? "Save" : "Create"}</p>
        </button>
      </form>
    </Modal>
  );
