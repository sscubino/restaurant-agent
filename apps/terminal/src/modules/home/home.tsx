...
const HomeModule = () => {
  ...
  const deleteOrder = async (orderId: number) => {
    try {
      ...
      if (data.ok) {
        const newValuesOrders = allOrders.filter((order: any) => order.id !== orderId)
  ...
  return (
    <div className="h-full w-full px-[25px] py-[50px] flex flex-col items-center gap-[20px]">
      <p className='font-bold text-[20px] text-gray-900 self-start '>Orders Managment</p>
      ...
      <div className="w-full flex flex-row items-center gap-[20px] ">
        ...
        <div className="">
          <button onClick={() => handleModal(true)} className="h-[46px] relative w-[150px] rounded-sm shadow-md  bg-gray-600 flex flex-row items-center justify-center">
            <MdTableRestaurant size={20} className="text-white absolute left-[12px]" />
            Tables
          </button>
        </div>
        <div className=""></div>
      ...
      <div className="flex p-[12px] shadow-md h-[500px] overflow-y-scroll flex-col w-full items-center bg-white rounded-[8px]">
        ...
        <div className="w-full h-full max-h-[100%] overflow-y-scroll flex flex-col items-center">
          {allOrders.map((order: any, index: number) => (
            <CardOrder key={index} deleteOrder={deleteOrder} orderData={order} />
          ))}
          <div ref={observerRef} className="h-10">
            {isFetching && <p className="text-gray-500">Loading more orders...</p>}
          </div>
          {
            allOrders.length === 0 && !isFetching &&
            <div className='text-center text-gray-500'> There are no orders available</div>
          }
        </div>
      </div>
