...
const ModalTables = ({ isOpen, onClose }: IModalProps) => {
  ...
  const changeStatusTable = async (id: number) => {
    ...
    try {
      ...
      if (tableIndex !== -1) {
        ...
        updatedTables[tableIndex].isAvailable =
          !updatedTables[tableIndex].isAvailable;
        setTables(updatedTables);
        await api.tables.update(id, {
          isAvailable: updatedTables[tableIndex].isAvailable,
        });
      }
    } catch (error) {
  ...
  const deleteSelectedTables = async () => {
    try {
      ...
      setTables(
        (prev) =>
          prev?.filter((table) => !selectedTables.includes(table.id)) || []
      );
      setSelectedTables([]);
  ...
  const CountAvailable = () => {
    return tables.filter((item) => item.isAvailable === true).length;
  };

  const addNewTable = (newItem: ITables) => {
    setTables((prevState) => [...prevState, newItem]);
  };
        ...
        <div className="w-full flex flex-col">
          <div className="w-full flex flex-row justify-between items-start my-[20px]">
            ...
            <button
              ...
              className="bg-black w-[100px] py-[4px] rounded-sm shadow-md"
            >
              Add Table
            </button>
          </div>
